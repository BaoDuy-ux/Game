...
public class DatabaseHelper
{
    ...
    public static int? LoginUser(string email, string password)
    {
        try
        {
            ...
            {
                connection.Open();
                string query = "SELECT user_id, password_hash FROM users WHERE email = @email";
    ...
    // Tạo farm mặc định
    private static void CreateFarm(int userId)
    {
        try
        {
            using (var connection = new MySqlConnection(connectionString))
        ...
        {
            Console.WriteLine($"Lỗi tạo farm: {ex.Message}");
        }
    }
    ...
    public static bool SaveChatMessage(int userId, string username, string message)
    {
        try
        {
            using (var connection = new MySqlConnection(connectionString))
            {
                ...
                Console.WriteLine($"✅ Đã lưu tin nhắn: userId={userId}, username={username}, message={message}");
                return true;
            }
        }
        catch (Exception ex)
